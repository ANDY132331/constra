...
export async function exportReportPdf(input: PdfReportInput) {
  ...
  doc.text(companyName ?? "Constra", MARGIN, 12);

  doc.setTextColor(255, 255, 255);
  doc.setFontSize(9);
  doc.setFont("helvetica", "normal");
  doc.text("CONSTRUCTION MANAGEMENT REPORT", MARGIN, 19);

  doc.setTextColor(180, 180, 180);
  doc.setFontSize(8);
  const genDate = new Date().toLocaleDateString("en-CA", { dateStyle: "long" });
  doc.text(`Period: ${periodLabel}   |   Generated: ${genDate}`, MARGIN, 25);
  ...
  const totalPayAmt = [...payrollByWorker.values()].reduce((s, r) => s + r.pay, 0);

  if (payrollRows.length === 0) {
    doc.setTextColor(...MID);
    doc.setFontSize(8);
    doc.setFont("helvetica", "italic");
    doc.text("No hours recorded in this period.", MARGIN + 3, y + 5);
    y += 12;
  } else {
    autoTable(doc, {
      startY: y,
      ...
      body: payrollRows,
      foot: [["", "TOTAL", fmtHours(totalPayHours), fmt(totalPayAmt, currency)]],
  ...
  if (timesheetRows.length === 0) {
    ...
    doc.setFontSize(8);
    doc.setFont("helvetica", "italic");
    doc.text("No timesheet entries in this period.", MARGIN + 3, y + 5);
    y += 12;
  } else {
    autoTable(doc, {
      startY: y,
      margin: { left: MARGIN, right: MARGIN },
      head: [["Date", "Worker", "Project", "Clock In", "Clock Out", "Hours"]],
      body: timesheetRows.map((r) => [r.date, r.worker, r.project, r.clockIn, r.clockOut, fmtHours(r.hours)]),
      headStyles: { fillColor: DARK, textColor: [255, 255, 255], fontSize: 8, fontStyle: "bold" },
      bodyStyles: { fontSize: 7.5, textColor: MID },
      alternateRowStyles: { fillColor: [250, 250, 250] },
  ...
  const budgetRows = projects
    ...
    .map((p) => {
      ...
      const status = p.budget > 0 && p.spent > p.budget ? "Over Budget" : p.budget > 0 && p.spent / p.budget > 0.9 ? "At Risk" : "On Track";
      return [p.name, p.status, fmt(p.budget, currency), fmt(p.spent, currency), fmt(remaining, currency), pct, status];
    });
  ...
  if (budgetRows.length === 0) {
    ...
    doc.setFontSize(8);
    doc.setFont("helvetica", "italic");
    doc.text("No projects with budget data.", MARGIN + 3, y + 5);
    y += 12;
  } else {
    autoTable(doc, {
      startY: y,
      ...
      didParseCell(data) {
        if (data.column.index === 6 && data.section === "body") {
          ...
          if (val === "Over Budget") data.cell.styles.textColor = [239, 68, 68];
          else if (val === "At Risk") data.cell.styles.textColor = [245, 158, 11];
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setFontSize(7);
    doc.setFont("helvetica", "normal");
    doc.text(`${companyName ?? "Constra"} · Confidential · ${periodLabel}`, MARGIN, doc.internal.pageSize.getHeight() - 4);
    doc.text(`Page ${i} of ${pageCount}`, PAGE_W - MARGIN, doc.internal.pageSize.getHeight() - 4, { align: "right" });
  }
