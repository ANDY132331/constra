...
export function OfflineBanner() {
  ...
  useEffect(() => {
    const prev = prevPendingSync.current;
    prevPendingSync.current = pendingSync;
    // Fire toast when pending count drops to 0 after a real offline sync
    if (isOnline && pendingSync === 0 && prev > 0) {
      setJustSynced(true);
  ...
  if (!isOnline) {
    return (
      <div className="fixed bottom-24 lg:bottom-4 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2.5 bg-[#1a1a1a] border border-white/10 rounded-full px-4 py-2 shadow-2xl">
  ...
  if (justSynced) {
    return (
      <div className="fixed bottom-24 lg:bottom-4 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2.5 bg-[#1a1a1a] border border-green-500/20 rounded-full px-4 py-2 shadow-2xl">
