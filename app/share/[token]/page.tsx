  ...
  if (error || !project) {
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center gap-4 text-center px-4">
        ...
        <h1 className="text-[20px] font-bold text-white">Project not found</h1>
      ...
      <div className="max-w-2xl mx-auto px-4 py-10 space-y-8">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className={`text-[11px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full ${
              ...
              project.status === "completed" ? "bg-white/8 text-white/40" : "bg-blue-500/15 text-blue-400"
            }`}>
              {project.status}
            </span>
          </div>
          <h1 className="text-[28px] font-black text-white tracking-tight mt-2">{project.name}</h1>
          <div className="flex flex-wrap items-center gap-4 mt-2 text-[13px] text-white/50">
        ...
        <div className="bg-[#111111] border border-white/[0.06] rounded-2xl p-6">
          <div className="flex items-center justify-between mb-3">
            <span className="text-[13px] font-semibold text-white/60">Overall Progress</span>
            <span className="text-[28px] font-black text-white">{project.progress}%</span>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={label} className="text-center bg-white/[0.03] rounded-xl py-3">
                ...
                <p className="text-[10px] text-white/30 mt-0.5 font-semibold uppercase tracking-wider">{label}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        {project.tasks.length > 0 && (
          <div>
            <h2 className="text-[13px] font-bold text-white/40 uppercase tracking-widest mb-3">Work Items</h2>
                ...
                return (
                  <div key={task.id} className="flex items-center gap-3 px-4 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[13px] text-white/80 font-medium truncate">{task.name}</p>
