...
import {
  ...
  Download, Link2, Check, Copy, Trash2, Plus, X, MoreVertical,
} from "lucide-react";
  ...
  return (
    <div className="bg-[#080808]" style={{ minHeight: '100dvh' }}>
      ...
      >
        ...
        >
          <ChevronLeft size={20} />
          <span className="text-[13px] font-medium hidden sm:inline">Invoices</span>
        </button>

        <div className="flex items-center gap-2 flex-1 min-w-0 mx-1">
          <span className="font-mono text-[12px] text-white/50 tracking-wider truncate">{invoice.number}</span>
        ...
        <div className="flex items-center gap-1 shrink-0">
          <button
            ...
            className={`flex items-center gap-1.5 text-[12px] font-semibold px-3 py-1.5 rounded-full transition-colors disabled:opacity-40 ${
              isOverdue ? "text-red-400 bg-red-500/10 hover:bg-red-500/20" : "text-white/70 bg-white/[0.08] hover:bg-white/[0.12]"
            }`}
          >
            <Mail size={13} />
            ...
            {menuOpen && (
              <div className="absolute right-0 top-full mt-1.5 w-56 bg-[#1a1a1a] border border-white/[0.08] rounded-2xl shadow-2xl z-40 py-1.5 overflow-hidden">
                {invoice.status === "draft" && (
                  <button onClick={() => { updateInvoice(invoice.id, { status: "sent" }); toast.success("Marked as sent"); setMenuOpen(false); }}
                ...
                {(invoice.status === "sent" || invoice.status === "overdue") && (
                  <button onClick={() => { updateInvoice(invoice.id, { status: "paid" }); toast.success("Marked as paid"); setMenuOpen(false); }}
                ...
                {invoice.status === "sent" && (
                  <button onClick={() => { updateInvoice(invoice.id, { status: "overdue" }); toast.success("Marked as overdue"); setMenuOpen(false); }}
                ...
                <button
                  onClick={async () => { setMenuOpen(false); setPdfLoading(true); try { await exportInvoicePdf(invoice, currency, companyName, companyAddress, companyLogo, "#F5C400", "save", template); } finally { setPdfLoading(false); } }}
                  disabled={pdfLoading}
