...
function dashSVG() {
  ...
  <text x="136" y="156" text-anchor="middle" font-size="28" font-weight="900" fill="#F59E0B" font-family="system-ui">3</text>
  ...
  <circle cx="118" cy="267" r="20" fill="rgba(245,158,11,.12)" filter="url(#dgf)"/>
  <circle cx="118" cy="267" r="10" fill="rgba(245,158,11,.28)"/>
  <circle cx="118" cy="267" r="5" fill="#F59E0B"/>
  <circle cx="118" cy="267" r="20" fill="none" stroke="rgba(245,158,11,.25)" stroke-width="1.5">
  ...
  <rect x="10" y="378" width="27" height="27" rx="8" fill="rgba(245,158,11,.12)"/>
  <text x="24" y="396" text-anchor="middle" font-size="9" fill="#F59E0B" font-weight="700" font-family="system-ui">MJ</text>
  ...
  <text x="34" y="537" text-anchor="middle" font-size="16">🏠</text><text x="34" y="552" text-anchor="middle" font-size="7" fill="#F59E0B" font-family="system-ui">HOME</text>
...
function invoiceSVG() {
  ...
  <rect x="168" y="120" width="80" height="22" rx="11" fill="rgba(245,158,11,.1)" stroke="rgba(245,158,11,.25)" stroke-width="1"/>
  ...
  <text x="254" y="219" text-anchor="end" font-size="26" fill="#F59E0B" font-weight="900" font-family="system-ui">$14,012</text>
  ...
  <text x="254" y="463" text-anchor="end" font-size="18" fill="#F59E0B" font-weight="900" font-family="system-ui">$14,012</text>
...
const FEATURES = [
  ...
  { icon: FileText,      color: "#f59e0b", title: "Estimates & Invoices",              body: "Build itemized estimates, convert to invoices with one click, export branded PDFs." },
  ...
  { icon: Sparkles,      color: "#f59e0b", title: "AI Daily Brief",                   body: "AI-generated site briefing every morning — crew status, overdue tasks, what to tackle first." },
  ...
  useEffect(() => {
    ...
    if (cv) {
      ...
      const STARS = Array.from({ length: 130 }, () => ({
        ...
        z: Math.random() * .95 + .05, spd: Math.random() * .0005 + .00018,
        warm: Math.random() < .6,
      }));
      ...
      const draw = () => {
        if (cancelled) return;
        ctx.fillStyle = "rgba(5,5,12,.13)"; ctx.fillRect(0, 0, W, H);
        for (const s of STARS) {
          const sx = (s.x / s.z) * W * .5 + W * .5, sy = (s.y / s.z) * H * .5 + H * .5;
          if (sx < -5 || sx > W + 5 || sy < -5 || sy > H + 5) { s.x = (Math.random() - .5) * 2; s.y = (Math.random() - .5) * 2; s.z = 1; continue; }
          const pz = s.z - s.spd, px = (s.x / pz) * W * .5 + W * .5, py = (s.y / pz) * H * .5 + H * .5;
          const sz = Math.max(.3, (1 - s.z) * 3.2), al = Math.min(1, (1 - s.z) * 1.3 + .08);
          const col = s.warm ? `rgba(245,${Math.floor(140 + Math.random() * 90)},11,` : "rgba(255,255,255,";
          ctx.beginPath(); ctx.moveTo(px, py); ctx.lineTo(sx, sy); ctx.strokeStyle = col + (al * .4) + ")"; ctx.lineWidth = sz * .65; ctx.stroke();
          ctx.beginPath(); ctx.arc(sx, sy, sz, 0, Math.PI * 2); ctx.fillStyle = col + al + ")"; ctx.fill();
          s.z -= s.spd;
    ...
    if (phoneEl) {
      let angY = 0, angX = 5, hov = false, mox = 0, moy = 0, lastT = 0;
      const onMM = (e: MouseEvent) => { mox = e.clientX; moy = e.clientY; };
      document.addEventListener("mousemove", onMM);
      phoneEl.addEventListener("mouseenter", () => { hov = true; });
      phoneEl.addEventListener("mouseleave", () => { hov = false; });
      const loop = (ts: number) => {
        if (cancelled) return;
        const dt = Math.min(ts - lastT, 50); lastT = ts;
        if (!hov) { angY += dt * .024; angX += (5 - angX) * .04; }
        else { angX += (-(moy / window.innerHeight - .5) * 26 - angX) * .09; angY += ((mox / window.innerWidth - .5) * 34 - angY) * .09; }
        phoneEl.style.transform = `rotateX(${angX}deg) rotateY(${angY}deg)`;
        phoneRaf = requestAnimationFrame(loop);
      };
      phoneRaf = requestAnimationFrame(loop);
    }
    ...
    return () => {
      cancelled = true;
      cancelAnimationFrame(starRaf); cancelAnimationFrame(phoneRaf);
      document.body.classList.remove("reveal-active");
    };
  }, []);

  return (
    <div className="min-h-screen text-white overflow-x-hidden" style={{ background: "#05050d" }}>

      {/* ── Nav ────────────────────────────────────────────────────────────── */}
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 50, background: "rgba(5,5,13,.88)", borderBottom: "1px solid rgba(255,255,255,.055)", backdropFilter: "blur(20px)" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 24px", height: 56, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 30, height: 30, background: "linear-gradient(135deg,#F59E0B,#F97316)", borderRadius: 9, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 16px rgba(245,158,11,.22)" }}>
              <HardHat size={14} color="#000" strokeWidth={2.5} />
            </div>
            <span style={{ fontSize: 15, fontWeight: 900, letterSpacing: "-.02em" }}>Constra</span>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
            <Link href="/login" style={{ fontSize: 13, color: "rgba(255,255,255,.38)", textDecoration: "none", fontWeight: 500 }}>Sign In</Link>
            <Link href="/onboarding" style={{ background: "linear-gradient(135deg,#F59E0B,#F97316)", color: "#000", fontWeight: 800, fontSize: 13, padding: "8px 18px", borderRadius: 10, textDecoration: "none", display: "flex", alignItems: "center", gap: 6, boxShadow: "0 4px 20px rgba(245,158,11,.18)" }}>
          ...
          <div>
            ...
            <div className="cl-trust">
              <div className="cl-avs">
                {[["MJ","rgba(245,158,11,.15)","#F59E0B"],["SP","rgba(155,89,255,.15)","#9B59FF"],["DT","rgba(34,197,94,.15)","#22c55e"],["KL","rgba(239,68,68,.15)","#ef4444"]].map(([id,bg,c])=>(
      ...
      {/* ── Marquee trust strip ──────────────────────────────────────────── */}
      <div style={{ background: "#07071000", borderTop: "1px solid rgba(255,255,255,.04)", borderBottom: "1px solid rgba(255,255,255,.04)", padding: "18px 0", overflow: "hidden", position: "relative" }}>
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to right, #05050d 0%, transparent 12%, transparent 88%, #05050d 100%)", zIndex: 2, pointerEvents: "none" }} />
      ...
      {/* ── Stats ────────────────────────────────────────────────────────── */}
      <section className="reveal" style={{ padding: "100px 24px", background: "#06060e" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          <p style={{ textAlign: "center", fontSize: 11, fontWeight: 700, letterSpacing: ".2em", textTransform: "uppercase", color: "#F59E0B", marginBottom: 56 }}>Built for serious contractors</p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 2 }}>
            {[
              ...
              <div key={i} style={{ padding: "40px 32px", borderRadius: 20, background: "rgba(255,255,255,.025)", border: "1px solid rgba(255,255,255,.05)", textAlign: "center" }}>
                <div style={{ fontSize: 58, fontWeight: 900, lineHeight: 1, color: "#F59E0B", marginBottom: 10, fontVariantNumeric: "tabular-nums" }}>
      ...
      <section style={{ position: "relative", minHeight: 560, display: "flex", alignItems: "center", overflow: "hidden" }}>
        ...
        <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to right, #05050d 35%, rgba(5,5,13,.7) 65%, rgba(5,5,13,.4) 100%)" }} />
        <div className="reveal" style={{ position: "relative", maxWidth: 1000, margin: "0 auto", padding: "80px 24px" }}>
          <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: ".2em", textTransform: "uppercase", color: "#F59E0B", marginBottom: 20 }}>Built for the field</p>
          <h2 style={{ fontSize: "clamp(36px,5vw,64px)", fontWeight: 900, lineHeight: .92, letterSpacing: "-.04em", marginBottom: 24 }}>
            Every job site<br />
            is <span style={{ background: "linear-gradient(135deg,#F59E0B,#F97316)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>different.</span><br />
          ...
          <Link href="/onboarding" style={{ display: "inline-flex", alignItems: "center", gap: 8, color: "#F59E0B", fontWeight: 700, fontSize: 14, textDecoration: "none" }}>
      ...
      <section style={{ padding: "100px 24px", background: "#05050d" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(300px,1fr))", gap: 16 }}>
            {[
              ...
              { icon: "📄", color: "#F59E0B", glow: "rgba(245,158,11,.08)", border: "rgba(245,158,11,.18)", problem: "Chasing invoices for weeks", fix: "Build estimates in the field, convert to invoices in one tap, send professional PDFs directly to clients." },
      ...
      {/* ── Photo strip ───────────────────────────────────────────────────── */}
      <div style={{ padding: "0 0 80px", overflow: "hidden", background: "#05050d" }}>
      ...
      {/* ── Features grid ─────────────────────────────────────────────────── */}
      <section id="features" style={{ padding: "100px 24px", background: "#06060e", borderTop: "1px solid rgba(255,255,255,.04)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(280px,1fr))", gap: 12 }}>
            {FEATURES.map((f, i) => (
              <div key={f.title} className="feat-card reveal" style={{ padding: "24px 24px 26px", borderRadius: 20, border: "1px solid rgba(255,255,255,.055)", background: "#0b0b18", position: "relative", overflow: "hidden", cursor: "default", transitionDelay: `${(i % 6) * 35}ms` }}>
      ...
      {/* ── How it works ──────────────────────────────────────────────────── */}
      <section style={{ padding: "100px 24px", background: "#05050d", borderTop: "1px solid rgba(255,255,255,.04)" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: 32 }}>
            {[
              ...
              <div key={s.n} className="reveal" style={{ transitionDelay: `${i * 80}ms` }}>
                <div style={{ width: 44, height: 44, borderRadius: 14, border: "1px solid rgba(245,158,11,.2)", background: "rgba(245,158,11,.07)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 20 }}>
                  <span style={{ fontSize: 13, fontWeight: 900, color: "#F59E0B" }}>{s.n}</span>
                </div>
      ...
      {/* ── Trades ────────────────────────────────────────────────────────── */}
      <section style={{ padding: "60px 24px", background: "#06060e", borderTop: "1px solid rgba(255,255,255,.04)" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
      ...
      {/* ── Pricing ───────────────────────────────────────────────────────── */}
      <section style={{ padding: "100px 24px", background: "#05050d", borderTop: "1px solid rgba(255,255,255,.04)" }} id="pricing">
        <div style={{ maxWidth: 680, margin: "0 auto" }}>
          ...
          <div className="reveal" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 16 }}>
            {[
              ...
              <div key={c.name} style={{ padding: "20px 16px", borderRadius: 18, border: `1px solid ${c.dim ? "rgba(255,255,255,.05)" : "rgba(245,158,11,.22)"}`, background: c.dim ? "rgba(255,255,255,.02)" : "rgba(245,158,11,.04)", opacity: c.dim ? .5 : 1, textAlign: "center" }}>
                <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: ".1em", textTransform: "uppercase", color: c.dim ? "rgba(255,255,255,.25)" : "#F59E0B", marginBottom: 6 }}>{c.name}</p>
          ...
          <div className="reveal" style={{ padding: "40px 36px", borderRadius: 24, border: "1px solid rgba(245,158,11,.2)", background: "rgba(245,158,11,.04)", position: "relative" }}>
            <div style={{ position: "absolute", top: -13, left: "50%", transform: "translateX(-50%)", background: "linear-gradient(135deg,#F59E0B,#F97316)", color: "#000", fontSize: 10, fontWeight: 900, padding: "5px 16px", borderRadius: 100, letterSpacing: ".08em", textTransform: "uppercase" }}>Early Access</div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 20, marginBottom: 32 }}>
              ...
              <Link href="/onboarding" style={{ background: "linear-gradient(135deg,#F59E0B,#F97316)", color: "#000", fontWeight: 900, fontSize: 14, padding: "14px 28px", borderRadius: 14, textDecoration: "none", display: "flex", alignItems: "center", gap: 8, boxShadow: "0 12px 36px rgba(245,158,11,.2)" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px 24px" }}>
              {["Unlimited crew members","GPS verification on every clock-in","Estimates & professional invoices","Safety incident logging","AI Daily Brief","Crew messaging","Weather-aware scheduling","Equipment management","RFIs & punch lists","PDF export","15-language support","Works offline"].map(f => (
                <div key={f} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5, color: "rgba(255,255,255,.45)" }}>
                  <Check size={11} color="#F59E0B" style={{ flexShrink: 0 }} />{f}
                </div>
              ))}
            </div>
          </div>
      ...
      {/* ── Final CTA ─────────────────────────────────────────────────────── */}
      <section style={{ padding: "120px 24px", background: "#06060e", borderTop: "1px solid rgba(255,255,255,.04)", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", inset: 0, background: "radial-gradient(ellipse 60% 50% at 50% 100%, rgba(245,158,11,.06), transparent)", pointerEvents: "none" }} />
        <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 1, background: "linear-gradient(to right, transparent, rgba(245,158,11,.15), transparent)" }} />
        <div className="reveal" style={{ maxWidth: 560, margin: "0 auto", textAlign: "center", position: "relative" }}>
          <div style={{ width: 56, height: 56, background: "linear-gradient(135deg,#F59E0B,#F97316)", borderRadius: 18, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 28px", boxShadow: "0 16px 48px rgba(245,158,11,.22)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 14 }}>
            <Link href="/onboarding" style={{ background: "linear-gradient(135deg,#F59E0B,#F97316)", color: "#000", fontWeight: 900, fontSize: 15, padding: "16px 36px", borderRadius: 16, textDecoration: "none", display: "flex", alignItems: "center", gap: 8, boxShadow: "0 16px 52px rgba(245,158,11,.22)", width: "100%", maxWidth: 360, justifyContent: "center" }}>
      ...
      {/* ── Footer ────────────────────────────────────────────────────────── */}
      <footer style={{ borderTop: "1px solid rgba(255,255,255,.05)", padding: "36px 24px", background: "#05050d" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ width: 24, height: 24, background: "linear-gradient(135deg,#F59E0B,#F97316)", borderRadius: 7, display: "flex", alignItems: "center", justifyContent: "center" }}><HardHat size={11} color="#000" /></div>
